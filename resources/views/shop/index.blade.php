<x-app-layout title="Shop">
  <style>
    ...
    /* Hide any duplicate slider elements */
    #price-slider-container .slider + .slider {
        display: none !important;
    }

    /* Ensure proper slider styling */
    #price-slider-container .slider.slider-horizontal {
        width: 100%;
        height: 20px;
    }
    ...
    #price-slider-container .slider-selection {
        background: #000;
    }
          ...
          <div class="accordion-item mb-4">
            ...
            <div id="accordion-filter-price" class="accordion-collapse collapse show border-0"
              ...
              <div id="price-slider-container">
                  <input id="price-range-slider" class="price-range-slider" type="text" name="price_range" value="" 
        ...
        <div class="d-flex justify-content-between mb-4 pb-md-2">
          ...
          <div class="shop-acs d-flex align-items-center justify-content-between justify-content-md-end flex-grow-1">
            ...
            <select class="shop-acs__select form-select w-auto border-0 py-0 order-1 order-md-0" aria-label="Sort Items" name="orderby" id="orderby">
              ...
              <option value="1" {{$order == 1 ? 'selected':""}}>Date, New o Old</option>
                ...
                <div class="product-card mb-3 mb-md-4 mb-xxl-5">
                  <div class="pc__img-wrapper">
                    <div class="swiper-container background-img js-swiper-slider" data-settings='{"resizeObserver": true}'>
                      <div class="swiper-wrapper">
                        <div class="swiper-slide">
                          <a href="{{route('shop.product.detail',['product_slug'=>$product->slug])}}"><img loading="lazy" src="/uploads/products/{{$product->image}}" width="330" height="400" alt="{{$product->name}}" class="pc__img"></a>
                        </div>
                        <div class="swiper-slide">
                            @foreach (explode(',',$product->images) as $img)
                                <a href="{{route('shop.product.detail',['product_slug'=>$product->slug])}}"><img loading="lazy" src="/uploads/products/{{$img}}" width="330" height="400" alt="Cropped Faux leather Jacket" class="pc__img"></a>
                            @endforeach
                        </div>
                      </div>
                  ...
                  <div class="pc__info position-relative">
                    ...
                    <form action="{{route('whishlist.item.remove',['rowId'=>Cart::instance('whishlist')->content()->where('id',$product->id)->first()->rowId])}}" method="POST" >
                      @csrf
                      @method('DELETE') 
                    ...
                    @else
                    <form action="{{route('whishlist.add')}}" method="POST">
                      @csrf
                      <input type="hidden" name="id" value="{{$product->id}}">
                      <input type="hidden" name="name" value="{{$product->name}}">
                      <input type="hidden" name="price" value="{{$product->sale_price == ''?$product->regular_price : $product->sale_price}}">
  ...
  <script>
      document.addEventListener('DOMContentLoaded', function() {
          ...
          document.getElementById('pagesize').addEventListener('change', function() {
              document.getElementById('size').value = this.options[this.selectedIndex].value;
              document.getElementById('frmfilter').submit();
          });

          // Handle orderby change
          document.getElementById('orderby').addEventListener('change', function() {
              document.getElementById('order').value = this.options[this.selectedIndex].value;
              document.getElementById('frmfilter').submit();
          });
          ...
          // Initialize price range slider with duplicate prevention
          ...
          if (priceSlider) {
              ...
              $priceSlider.slider({
                  min: parseInt(priceSlider.dataset.sliderMin),
                  max: parseInt(priceSlider.dataset.sliderMax),
                  step: parseInt(priceSlider.dataset.sliderStep),
                  value: JSON.parse(priceSlider.dataset.sliderValue),
                  tooltip: 'hide',
                  formatter: function(value) {
                      return '$' + value;
                  }
              }).on('change', function(e) {
                  const [min, max] = e.value.newValue || e.value;
                  document.getElementById("hdnMinPrice").value = min;
                  document.getElementById("hdnMaxPrice").value = max;
                  
                  // Update displayed prices
                  document.querySelector('.price-range__min').textContent = `$${min}`;
                  document.querySelector('.price-range__max').textContent = `$${max}`;
              ...
              // Clean up any duplicate elements the plugin might have created
              ...
              if (sliderContainer) {
                  ...
                  if (sliders.length > 1) {
                      // Keep only the first slider element
